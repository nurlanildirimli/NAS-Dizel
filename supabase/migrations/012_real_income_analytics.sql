-- 012: real analytics from detailed service line items.

create or replace function public.get_income_analytics(period_key text default 'month')
returns jsonb
language plpgsql
stable
as $$
declare
  v_start timestamptz := public.report_period_start(period_key);
  v_today_start timestamptz := date_trunc('day', now());
  v_month_start timestamptz := date_trunc('month', now());
  v_year_start timestamptz := date_trunc('year', now());
begin
  return jsonb_build_object(
    'period_key', period_key,
    'daily_income', coalesce((
      select jsonb_agg(row_to_json(row_data) order by row_data.day)
      from (
        select
          to_char(days.day, 'YYYY-MM-DD') as day,
          coalesce(sum(case when sr.id is not null then p.paid_amount else 0 end), 0)::numeric(12,2) as amount
        from generate_series(v_month_start, date_trunc('day', now()), interval '1 day') as days(day)
        left join public.payments p on p.is_deleted = false
          and p.payment_date >= days.day
          and p.payment_date < days.day + interval '1 day'
        left join public.service_records sr on sr.id = p.service_id
          and sr.is_deleted = false
        group by days.day
      ) row_data
    ), '[]'::jsonb),
    'monthly_income', coalesce((
      select jsonb_agg(row_to_json(row_data) order by row_data.month)
      from (
        select
          to_char(months.month, 'YYYY-MM') as month,
          coalesce(sum(case when sr.id is not null then p.paid_amount else 0 end), 0)::numeric(12,2) as amount
        from generate_series(date_trunc('month', now()) - interval '11 months', date_trunc('month', now()), interval '1 month') as months(month)
        left join public.payments p on p.is_deleted = false
          and p.payment_date >= months.month
          and p.payment_date < months.month + interval '1 month'
        left join public.service_records sr on sr.id = p.service_id
          and sr.is_deleted = false
        group by months.month
      ) row_data
    ), '[]'::jsonb),
    'top_models', coalesce((
      select jsonb_agg(row_to_json(row_data) order by row_data.total_amount desc)
      from (
        select
          concat(sr.injector_company, ' ', sr.injector_code) as label,
          count(distinct sr.id)::integer as service_count,
          coalesce(sum(sli.total_price), 0)::numeric(12,2) as total_amount
        from public.service_records sr
        left join public.service_line_items sli on sli.service_id = sr.id
        where sr.is_deleted = false
          and sr.service_date >= v_start
        group by sr.injector_company, sr.injector_code
        order by total_amount desc, service_count desc
        limit 5
      ) row_data
    ), '[]'::jsonb),
    'category_totals', coalesce((
      select jsonb_agg(row_to_json(row_data) order by row_data.item_type)
      from (
        select
          sli.item_type,
          coalesce(sum(sli.total_price), 0)::numeric(12,2) as total_amount,
          coalesce(sum(sli.quantity), 0)::integer as quantity
        from public.service_line_items sli
        join public.service_records sr on sr.id = sli.service_id
        where sr.is_deleted = false
          and sr.service_date >= v_start
        group by sli.item_type
      ) row_data
    ), '[]'::jsonb),
    'discount_total', coalesce((
      select sum(sr.discount_amount)::numeric(12,2)
      from public.service_records sr
      where sr.is_deleted = false
        and sr.service_date >= v_start
    ), 0),
    'today_income', coalesce((
      select sum(p.paid_amount)::numeric(12,2)
      from public.payments p
      join public.service_records sr on sr.id = p.service_id
      where p.is_deleted = false
        and sr.is_deleted = false
        and p.payment_date >= v_today_start
    ), 0),
    'month_income', coalesce((
      select sum(p.paid_amount)::numeric(12,2)
      from public.payments p
      join public.service_records sr on sr.id = p.service_id
      where p.is_deleted = false
        and sr.is_deleted = false
        and p.payment_date >= v_month_start
    ), 0),
    'year_income', coalesce((
      select sum(p.paid_amount)::numeric(12,2)
      from public.payments p
      join public.service_records sr on sr.id = p.service_id
      where p.is_deleted = false
        and sr.is_deleted = false
        and p.payment_date >= v_year_start
    ), 0)
  );
end;
$$;

create or replace function public.get_report_summary(
  report_key text,
  period_key text default 'month'
)
returns jsonb
language plpgsql
stable
as $$
declare
  v_start timestamptz := public.report_period_start(period_key);
begin
  if report_key = 'income' then
    return coalesce((
      select jsonb_agg(row_to_json(row_data))
      from (
        select
          'Gəlir' as label,
          coalesce(sum(p.paid_amount), 0)::numeric(12,2) as amount,
          count(*)::integer as count
        from public.payments p
        join public.service_records sr on sr.id = p.service_id
        where p.is_deleted = false
          and sr.is_deleted = false
          and p.payment_date >= v_start
      ) row_data
    ), '[]'::jsonb);
  elsif report_key = 'vehicles' then
    return coalesce((
      select jsonb_agg(row_to_json(row_data))
      from (
        select
          v.license_plate as label,
          v.brand,
          count(sr.id)::integer as service_count,
          coalesce(sum(sr.final_total), 0)::numeric(12,2) as total_amount,
          coalesce(sum(sr.remaining_amount), 0)::numeric(12,2) as remaining_amount
        from public.vehicles v
        left join public.service_records sr on sr.vehicle_id = v.id and sr.is_deleted = false
        where v.is_deleted = false
        group by v.id
        order by service_count desc, total_amount desc
        limit 50
      ) row_data
    ), '[]'::jsonb);
  elsif report_key = 'injectors' then
    return coalesce((
      select jsonb_agg(row_to_json(row_data))
      from (
        select
          sr.injector_company as company,
          sr.injector_code as code,
          count(distinct sr.id)::integer as service_count,
          coalesce(sum(sli.total_price), 0)::numeric(12,2) as line_item_amount,
          avg(sr.final_total)::numeric(12,2) as average_amount
        from public.service_records sr
        left join public.service_line_items sli on sli.service_id = sr.id
        where sr.is_deleted = false
          and sr.service_date >= v_start
        group by sr.injector_company, sr.injector_code
        order by line_item_amount desc, service_count desc
        limit 50
      ) row_data
    ), '[]'::jsonb);
  elsif report_key = 'payments' then
    return coalesce((
      select jsonb_agg(row_to_json(row_data))
      from (
        select
          sr.payment_status as status,
          count(*)::integer as service_count,
          coalesce(sum(sr.final_total), 0)::numeric(12,2) as final_total,
          coalesce(sum(sr.paid_amount), 0)::numeric(12,2) as paid_amount,
          coalesce(sum(sr.remaining_amount), 0)::numeric(12,2) as remaining_amount
        from public.service_records sr
        where sr.is_deleted = false
          and sr.service_date >= v_start
        group by sr.payment_status
        order by sr.payment_status
      ) row_data
    ), '[]'::jsonb);
  elsif report_key = 'services' then
    return coalesce((
      select jsonb_agg(row_to_json(row_data))
      from (
        select
          sr.service_date,
          v.license_plate,
          v.brand,
          sr.injector_company,
          sr.injector_code,
          sr.final_total,
          sr.payment_status
        from public.service_records sr
        join public.vehicles v on v.id = sr.vehicle_id
        where sr.is_deleted = false
          and sr.service_date >= v_start
        order by sr.service_date desc
        limit 100
      ) row_data
    ), '[]'::jsonb);
  elsif report_key = 'parts' then
    return coalesce((
      select jsonb_agg(row_to_json(row_data))
      from (
        select
          sli.item_type,
          sli.item_name,
          sli.option_name,
          sum(sli.quantity)::integer as quantity,
          coalesce(sum(sli.total_price), 0)::numeric(12,2) as total_amount
        from public.service_line_items sli
        join public.service_records sr on sr.id = sli.service_id
        where sr.is_deleted = false
          and sr.service_date >= v_start
          and sli.item_type in ('labor', 'part', 'extra')
        group by sli.item_type, sli.item_name, sli.option_name
        order by total_amount desc, quantity desc
        limit 50
      ) row_data
    ), '[]'::jsonb);
  elsif report_key = 'price_changes' then
    return coalesce((
      select jsonb_agg(row_to_json(row_data))
      from (
        select
          sr.service_date,
          v.license_plate,
          sli.item_type,
          sli.item_name,
          sli.option_name,
          sli.default_unit_price,
          sli.actual_unit_price,
          sli.quantity,
          sli.total_price,
          (sli.actual_unit_price - sli.default_unit_price)::numeric(12,2) as difference
        from public.service_line_items sli
        join public.service_records sr on sr.id = sli.service_id
        join public.vehicles v on v.id = sr.vehicle_id
        where sr.is_deleted = false
          and sr.service_date >= v_start
          and sli.price_changed = true
        order by sr.service_date desc
        limit 100
      ) row_data
    ), '[]'::jsonb);
  elsif report_key = 'debts' then
    return coalesce((
      select jsonb_agg(row_to_json(row_data))
      from (
        select
          v.license_plate,
          v.brand,
          v.phone,
          sr.remaining_amount,
          sr.service_date
        from public.service_records sr
        join public.vehicles v on v.id = sr.vehicle_id
        where sr.is_deleted = false
          and sr.remaining_amount > 0
        order by sr.remaining_amount desc
        limit 100
      ) row_data
    ), '[]'::jsonb);
  elsif report_key = 'problem_vehicles' then
    return coalesce((
      select jsonb_agg(row_to_json(row_data))
      from (
        select
          v.license_plate,
          v.brand,
          v.phone,
          v.problem_reason,
          coalesce(sum(sr.remaining_amount), 0)::numeric(12,2) as remaining_amount,
          max(sr.service_date) as last_service_date
        from public.vehicles v
        left join public.service_records sr on sr.vehicle_id = v.id and sr.is_deleted = false
        where v.is_deleted = false
          and v.is_problem_customer = true
        group by v.id
        order by remaining_amount desc, last_service_date desc nulls last
        limit 100
      ) row_data
    ), '[]'::jsonb);
  end if;

  raise exception 'invalid report key';
end;
$$;
