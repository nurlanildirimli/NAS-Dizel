-- Text-based service drafts and professional service note persistence.
...
create index if not exists idx_service_drafts_status_updated_at
  on public.service_drafts(status, updated_at desc);
...
begin
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
    alter table public.service_drafts replica identity full;
    ...
    if not exists (
      ...
      where pubname = 'supabase_realtime'
        ...
        and tablename = 'service_drafts'
    ) then
      alter publication supabase_realtime add table public.service_drafts;
    end if;
  end if;
end;
$realtime$;
...
as $$
declare
  v_service jsonb := payload -> 'service';
...
begin
  ...
  values (
    ...
    nullif(v_service ->> 'work_performed', ''),
    nullif(v_service ->> 'technical_notes', ''),
    v_injector_count,
    v_model_id,
    v_model_company,
    v_model_code,
